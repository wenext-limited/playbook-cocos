...
`query_ck.py` 优先读取环境专用的 `$CLICKHOUSE_PROD_PASSWORD` / `$CLICKHOUSE_TEST_PASSWORD`，再回退 `$CLICKHOUSE_PASSWORD`，最后读取 `~/.wenext/clickhouse.json`；Skill 内不保存凭据。配置文件必须为 `600`，生产与测试凭据不同时按 `prod` / `test` 分段，并可在各自分段内覆盖 host、port、user、secure。脚本强制只读并限制最多返回 10,000 行；只允许 `SELECT`、`WITH ... SELECT`、`SHOW`、`DESCRIBE`、`EXPLAIN` 和 `EXISTS`，禁止任何写入或管理语句。
