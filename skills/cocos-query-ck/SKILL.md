...
```bash
python3 <skill-dir>/scripts/query_ck.py --database yoki \
...
用户只要 SQL 时使用 `--validate-only` 校验并输出 SQL，不连接数据库。生产失败时读取错误并修正；只有网络/DNS 瞬态错误可以重试，不能静默切换测试环境。
...
先给结论和小表格，再报告：环境、App/database、游戏/gameType、时间范围、事件/action、过滤条件、聚合函数以及分子/分母。
...
`query_ck.py` 优先从 `$CLICKHOUSE_PASSWORD` 读取密码，否则读取 `~/.wenext/clickhouse.json`；Skill 内不保存凭据。配置文件必须为 `600`，可按 `prod` / `test` 分段，并可覆盖 host、port、user、secure。脚本强制只读并限制最多返回 10,000 行；只允许 `SELECT`、`WITH ... SELECT`、`SHOW`、`DESCRIBE`、`EXPLAIN` 和 `EXISTS`，禁止任何写入或管理语句。
