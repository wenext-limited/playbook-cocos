...
| `<target-project>/.claude/cocos-feature-migration/<feature-slug>/迁移清单.md` | 判断目标迁移进度、resume 状态、引用的源分析 | 是 |
...
| `file=None` 处理 | 关键业务资源解析正常时，将疑似内建资源标为 note，并要求 L3 复核 | 直接当作通过且不记录，或直接当作阻塞 |
