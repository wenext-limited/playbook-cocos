...
目标分支确认菜单默认不必为了展示所有可用项而预热全部远程引用；只能展示已经由本地信息确认的策略、固定自定义输入格式和暂停项。远程相关选项可写为“选择后校验”。若用户选择后校验失败，阻塞并说明原因。
...
2. **目标非默认分支确认时，菜单只展示当前可直接执行的策略**，并在菜单前用极简摘要说明当前分支、默认迁移分支和关键校验结果。当前本地分支总是可作为“从当前本地分支创建”的本地基线；当前不可执行的策略（例如未检测到已有迁移分支）不要进入编号列表；最多在“不可用摘要”中一行说明。
...
- 不得只展示“从 origin/main 创建 / 继续当前本地分支”两个选项；若当前本地分支、已有迁移分支可执行，应展示；若不可执行，不要进入编号列表。固定保留 `base=origin/xxx`、`branch=feature/xxx` 和 `暂停`。
