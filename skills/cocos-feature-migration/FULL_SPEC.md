...
> 维护提示：当前主流程以 `SKILL.md` 与 `guides/` 为准。凡涉及用户确认菜单（目标分支策略、迁移范围/功能边界、源分析缓存处理、保真风险确认等），都必须使用 `A/B/C/...` 大写字母选项，并提示“直接回复字母或文本即可”。本历史完整规范中若仍出现 `1/2/3/...` 数字菜单示例，一律视为过期示例，不得用于实际对话。
...
| `target-capability-analyzer` | 第 5 步前半：目标同名/同职责能力、公共能力复用、代码/职责差异 | `05-目标差异分析.md`、`目标差异摘要.compact.md` | 否 |
...
完整规范同步主入口与分阶段 guide 的调度增强模型：本 skill 必须采用 `checkpoint + state bootstrap + heartbeat + watchdog + artifact harvest + DAG transition`，确保主控不依赖用户追问或 agent 自述推进。
...
1. **Checkpoint**：主控必须维护 `<target_migration_dir>/controller-checkpoint.compact.md`，只保存当前调度快照，不保存完整代码/资源事实；默认不超过 80 行。
2. **State bootstrap**：主控启动任何阶段 agent 前，必须先预创建或更新该阶段 `state_compact_artifact`，写入 `status: running`、`phase_runtime`、`required_artifacts_pending`、`heartbeat_path`、`watchdog_status`。
3. **Heartbeat**：agent 启动后必须尽快写 `<source_analysis_dir-or-target_migration_dir>/logs/heartbeat/<phase>-<agent>.heartbeat.json`，并在关键 step 更新 `current_step`、`completed_steps`、`last_output_path`、`updated_at`；heartbeat 默认不超过 40 行，只保存调度信息，不保存完整证据。
4. **Watchdog**：主控启动 agent 后必须安排 bounded watchdog；优先 `Monitor`，不可用时使用 `Bash(run_in_background=true)` one-shot fallback。watchdog 只输出 checkpoint / completed / soft_timeout / failed 单行事件，不得输出完整 compact、完整日志或完整资源清单。
5. **Artifact harvest**：主控被 `agent_result`、`idle_notification`、watchdog 事件、用户状态询问或 resume 唤醒时，统一检查 required artifacts + state compact；完整则推进，缺失则追问一次 / 重启一次 / 阻塞。
6. **DAG transition**：每个阶段完成后，主控必须根据显式 transition 启动下一阶段或进入用户确认门禁，不得停在隐式等待。默认 transition：`01 -> 02 -> 03 -> 04 -> 05x -> 05a -> 05b+05c -> 05-merge -> 06 -> 07-verifier -> final-report`。
7. **上下文预算**：checkpoint <= 80 行、heartbeat <= 40 行、watchdog 单事件 1 行、state compact 默认 <= 200 行；完整事实写 evidence compact / 步骤 md / logs。

主控收到 idle 但无 `agent_result` 时，不得等待；必须立即 artifact harvest。若 required artifacts + state compact 完整，记录 `completed_with_agent_output_missing` 并执行 DAG transition。
...
10. **主控采用“关键门禁强复核 + 普通阶段抽样复核”。** 必须强复核：入口/边界确认、目标分支确认、第 5 步保真风险、`migration-applier` 改动清单与关键自检、`static-verifier` 的阻塞/修复建议、最终 open confirmations。其他阶段默认信任 compact；只有 compact 缺失、状态冲突、证据不足、出现 open confirmation 或 agent 越权风险时，才下钻读取完整步骤 md。
...
### State Bootstrap / Heartbeat / Watchdog / Artifact Harvest 协议

#### State Bootstrap 最小模板
...
artifact harvest 默认只检查存在性并读取 state compact；只有缺失、冲突、open confirmation、越权风险或用户要求细节时，才读取 evidence compact / 步骤 md / logs。
...
主控或 final-report-writer 汇总 timing 时必须计算 step 覆盖率：
...
若 `total_duration_seconds >= 120` 且 `coverage_ratio < 0.5`，必须在 compact 与 `使用效果监控.md` 记录 `step_granularity_insufficient: true`，并说明 `timing_gap_reason: step durations do not cover agent total runtime`。不得把这种 agent 的 slowest step 当作完整可观测慢点。
...
pending_confirmations:
  - id: "feature-boundary-001"
    ...
    status: open | closed | excluded | resolved-by-evidence
    opened_by_stage:
    opened_by_agent:
    source_file:
    evidence_paths:
...
`checkpoint + state bootstrap + heartbeat + watchdog + artifact harvest + DAG transition` 不得增加 main 上下文负担：

- checkpoint 默认不超过 80 行；
- heartbeat 默认不超过 40 行；
- watchdog 每条事件必须为 1 行；
- state compact 默认不超过 200 行；
- event log 默认不读，冲突/恢复时最多读最近 80 行；
- artifact harvest 默认只做 required artifacts 存在性检查 + 读取当前 state compact；
- 禁止 heartbeat / watchdog 输出完整资源清单、完整搜索结果、完整 CLI 输出。
...
若项目不适合写入 `.cocos-migration-cache/`，可写入本轮 `.claude/cocos-feature-migration/<feature-slug>/logs/cache/`，但必须在 manifest 中记录实际路径。
...
- 当前分支：`<current-branch>`
- 默认迁移分支：`feature/migration_<feature-slug>`
- 可用基线：`origin/main`、当前本地分支 `<current-branch>`
- 不可用：<仅列必要摘要；没有则省略本行>
...
第 5 步采用**私有产物 + 单点合并**模式，避免多个 agent 并发覆盖同一 `05-目标差异分析.md` 或 `目标差异摘要.compact.md`。
...
#### 5.0.shared 目标共享检索包（推荐加速）

在启动 `target-capability-analyzer`、`fidelity-risk-analyzer`、`resource-migration-planner` 前，若目标项目体量较大或第 5 步需要 9 agent 并行，应先生成或刷新轻量共享检索包：

```text
05x-target-shared-search.compact.json
```

该文件只记录高价值命中：request / native / event / KV / i18n / prefab / resource dirs / UI common / activity gating 等路径、行号、少量摘要和证据路径。三个第 5 步 agent 必须优先读取该共享包，避免重复全局搜索；但它不能替代各 agent 的私有深查和风险判断。
...
- `copy_files`：源路径、目标路径、ok / conflict / overwrite-risk / missing-source；
- `rewrite_imports`：待改 import、目标路径、是否可解析；
- `asset_mapping`：source uuid / path、target path、copy / reuse / rebind / skip；
- `prefab_rebind`：prefab、node、property、source uuid、target uuid、状态；
- `target_conflicts`：同名文件、同名资源、UUID 冲突、bundle 目录差异。
...
- **L1：静态结构验证（默认最高验证等级）**
  ...
  - `static_status_breakdown`：代码、UIConfig/Event/Protocol、业务资源 deps、脚本绑定、公共 UUID、builtin-like、入口视觉、动态资源、职责等价、保真等维度状态；
...
第 7 步还必须对 Prefab 中涉及的公共资源 UUID 进行独立审计，不能只依赖 deps 未报 missing 作为完整闭合证据。
...
final_status_synthesis:
  ...
  downgrade_reasons:
    ...
    - entry_visual_integration_partial
```
...
- 但若入口视觉资源仍为占位、公共资源 UUID 改绑未全量审计、或 builtin-like / unknown unresolved 只能要求人工复核，应按 `static_status_breakdown` 降级为 `partial-pass-static`，不得只因业务资源 missing=0 就给 `static-pass`。
