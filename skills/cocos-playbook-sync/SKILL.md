...
description: 同步本地 playbook-cocos 仓库与 GitHub 远程。用于用户要求更新本地知识库、上传本地修改、或把本地 playbook-cocos 变更提交为 PR 时。流程包含环境检查、工作区变更判断、无本地修改时仅快进更新、有本地修改时列出变更并让用户选择丢弃或进入 PR 提交流程。
tags: [cocos, git, github, pull-request, sync, playbook]
inputs: [本地 playbook-cocos 仓库状态, 用户对本地修改的处理选择, 分支名, PR 标题和描述]
outputs: [更新后的本地 main, 或 feat 分支和 GitHub Pull Request]
---
...
如果 `git status --short` 没有输出，说明工作区和暂存区都干净。此时只做更新：
