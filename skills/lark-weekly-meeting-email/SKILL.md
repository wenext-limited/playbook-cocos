...
只收录明确的线上问题、事故、生产故障、客诉、发布后问题及其解决经验。
...
- 采用单列文本流，使用 `<p>`、`<div>`、`<strong>`、`<br>`、`<hr>`、`<a>` 和简单 `<table>` 组织内容。
- 实际邮件正文必须放在唯一的 `id="mailContent"` 容器中；复制按钮和状态提示必须位于该容器之外。
- 不设置页面背景、内容卡片、最大宽度、居中容器或响应式布局。
- 章节间隔依靠块级元素、空段落或 `<hr>`，不能依赖 `margin`、`padding` 保持结构。
...
- 业务需求使用 APP / 项目分组标题，事项文本块内不重复 APP 标签。
...
- `border` 仅用于简单表格或 `<hr>`；不得用边框模拟事项卡片、左侧色条或分区容器。
...
5. 对 `data.cleaned_html` 执行粘贴安全检查：
   ...
   - 仅保留文字颜色、字号、粗体、链接、分隔线和简单表格等基础格式。
...
8. 最终检查：
   ...
   - 同一 APP 只出现一个分组标题，该 APP 的全部业务事项都在该分组内。
